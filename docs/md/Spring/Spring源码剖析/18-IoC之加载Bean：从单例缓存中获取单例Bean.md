<h1 align="center">IoC 之加载 Bean：从单例缓存中获取单例 Bean</h1>
...
在上篇博客 **《IoC 之开启 Bean 的加载》**中提到过，Spring 对单例模式的 bean 只会创建一次。后续，如果再获取该 Bean ，则是直接从单例缓存中获取，该过程就体现在 `#getSingleton(String beanName)` 方法中。代码如下：
