...
- `<3>` 处，判断资源路径 `location` 是绝对路径还是相对路径。详细解析，见 [「2.1 判断路径」](#2.1 判断路径) 。
  - `<4>` 处，如果是绝对路径，则调递归调用 Bean 的解析过程，进行另一次的解析。详细解析，见 [「2.2 处理绝对路径」](#2.2 处理绝对路径) 。
  - `<5>` 处，如果是相对路径，则先计算出绝对路径得到 Resource，然后进行解析。详细解析，见 [「2.3 处理相对路径」](#2.3 处理相对路径) 。
- `<6>` 处，通知监听器，完成解析。
