<h1 align="center">IoC 之解析 bean 标签：constructor-arg、property、qualifier</h1>
...
上篇博客 《 IoC 之解析 标签：meta、lookup-method、replace-method》 分析了 `meta`、`lookup-method`、`replace-method` 三个子元素。这篇博客，我们来分析 `constructor-arg`、`property`、`<qualifier>` 三个子元素。
...
2. 在 `<2>` 处，调用 `#parsePropertyValue(Element ele, BeanDefinition bd, String propertyName)` 方法，解析 `constructor-arg` 子元素，返回结果值。详细解析，见 [「1.4 parsePropertyValue」](#1.4 parsePropertyValue) 。
...
4. 在 `<4>` 处，如果子元素不为空，则调用 `#parsePropertySubElement(Element ele, BeanDefinition bd)` 方法，对子元素进一步解析。详细解析，见 [「1.5 parsePropertySubElement」](#1.5 parsePropertySubElement) 中。
...
> `<qualifier>` 标签在实际情况下用的比较少

## 3.1 示例

见 [《Spring 注解实现Bean依赖注入之 @Qualifier》](https://blog.csdn.net/lovin_fang/article/details/78537547?locationNum=4&fps=1) 文章。
