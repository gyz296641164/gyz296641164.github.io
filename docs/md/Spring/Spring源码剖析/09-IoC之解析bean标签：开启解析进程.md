<h1 align="center">IoC 之解析 bean 标签：开启解析进程</h1>
...
1. 调用
   ...
   - 详细解析，见 [「2. parseBeanDefinitionElement」](#2. parseBeanDefinitionElement) 。
...
- `<4>` 处，调用 `#parseBeanDefinitionElement(Element ele, String beanName, BeanDefinition containingBean)` 方法，对属性进行解析并封装成 AbstractBeanDefinition 实例 `beanDefinition` 。详细解析，见 [「2.1 parseBeanDefinitionElement」](#2.1 parseBeanDefinitionElement) 。
